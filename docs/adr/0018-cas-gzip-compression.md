...
**Context:** Doc 08 and the Phase 3 playbook name **zstd** for CAS object compression. Implementation exposed a conflict: `node:zlib` gained zstd only in Node ≥23.8, KEEL's supported floor is Node 20 (CI matrix runs 20 and 22), and a native zstd package violates the frozen native-dependency budget (Doc 11 §7, `better-sqlite3` only) — the same class of conflict as ADR-017.
